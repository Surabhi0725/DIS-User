...
import sgsits.cse.dis.user.feignClient.ResponseMessage;
...
import sgsits.cse.dis.user.repo.GroupRepository;
...
public class CalendarServicesImpl implements CalendarServices {
  ...
  public List<Holiday> getPublicHolidays() {
    List<Holiday> holiday = (List<Holiday>) holidayRepository.findAll();
    return holiday;
  }

  @Override
  public List<Holiday> addPublicHoliday(List<Holiday> holidays){
    for(Holiday ho : holidays) {
      holidayRepository.save(ho);
    }
    return  holidays;
  }

  @Override
  public Event addEvent(EventDto event, MultipartFile[] files)
  ...
  @Override
  public Event updateEvent(EventDto event, String eventId, MultipartFile[] files)
  ...
  public List<Event> getMyEvents(String participantId) {
    ...
    List<Event> eventParticipantsList = eventRepository.findAllByParticipants_ParticipantId(
            participantId);
  ...
  private void sendMeetingInvites(ArrayList<String> username_list, String mail_type, Event event)
          throws UnknownHostException, MessagingException, SQLException {
    String type;
    ArrayList<String> mailing_list = new ArrayList<String>();
  ...
  private void generateNotification(Event event, String notification_type,
    ...
    newNotification.setHeading(
            type + " : " + event.getTitle() + "@ " + event.getStartDate().toString());
    ...
    if (files != null) {
      if (files.length > 0) {
        ...
        for (MultipartFile aFile : files) {
          ...
          System.out.println(aFile instanceof MultipartFile);
          ResponseEntity<ResponseMessage> message = taskAttachment.uploadFile(aFile);
